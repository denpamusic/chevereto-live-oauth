...
$route = function($handler) {
	try {
		...
		if(!in_array($doing, ['google', 'facebook', 'twitter', 'vk', 'live'])) {
		...
		switch($doing) {
			...
			case 'live':
				$live = [
					'id' 		=> CHV\getSetting('live_client_id'),
					'secret'	=> CHV\getSetting('live_client_secret')
				];

				$client = new Stevenmaguire\OAuth2\Client\Provider\Microsoft([
					'clientId'     => $live['id'],
				...
				if ( isset($_GET['code']) ) {
					...
					try {
						...
						$connect_user = [
							'id'		=> $user->getId(),
							'username'	=> G\sanitize_string(G\unaccent_string($user->getFirstname() . $user->getLastname()), true, true),
						...
						throw new Exception('Live connect error:' . $e->getMessage(), 400);
					}
				} else {
